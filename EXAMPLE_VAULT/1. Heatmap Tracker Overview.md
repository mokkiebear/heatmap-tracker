...
```dataviewjs
// 1. Change the path to the folder with your notes
const FOLDER_PATH = 'daily notes';

// 2. Create trackerData object with your settings
const trackerData = {
    year: 2024, // optional, remove this line to autoswitch year
    entries: [], // empty by default, will be filled with data from your notes. Or, you can prefill it with your data
    separateMonths: true, // optional, set to false if you don't want to separate months
    colorScheme: {
        // paletteName: "default",
        customColors: [
            "rgb(255, 0, 0)",
...
for(let page of dv.pages(`"${FOLDER_PATH}"`).where(p=>p.woke)){
console.log(page)
