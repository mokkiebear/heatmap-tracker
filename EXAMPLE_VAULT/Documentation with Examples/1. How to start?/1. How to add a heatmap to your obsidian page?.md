...
**Option 0 (the modal — recommended):**
Run **Insert Heatmap Tracker** from the command palette, or click the Heatmap Tracker icon in the left ribbon. Pick a property, optionally a folder and tags, and the plugin writes the codeblock for you — with a live preview and a count of matching notes while you fill it in. Any heatmap can be reopened in the same form later via the pencil button in its corner.

The options below are for cases the form doesn't cover: data that isn't in frontmatter, or values you need to compute.
