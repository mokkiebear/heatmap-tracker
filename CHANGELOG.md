...
## [Unreleased]
### Added
- The `heatmap-tracker` codeblock now reads frontmatter directly from Obsidian's metadata cache, so the plugin works without Dataview installed. Dataview is still used automatically when present — it also indexes inline fields (`steps:: 8420`), which the metadata cache does not see.
- A `heatmap-tracker` codeblock that cannot render now explains why in the note itself — no `property` set, invalid YAML, or an unexpected failure — instead of writing to the developer console and leaving an empty space.
- A hint under heatmaps whose query matched no notes, naming the property and path it looked for.
- A product website under `website/`, published to GitHub Pages at https://mokkiebear.github.io/heatmap-tracker/ — an overview of what the plugin does, an interactive demo heatmap, the install steps and the FAQ, with structured data so search and AI answer engines can index it.

### Fixed
- A codeblock rendered while Dataview is still starting up now waits for its index instead of falling back to frontmatter, which would have silently dropped inline fields.
- The plugin language is now applied on load, so codeblock messages and modals are translated before any heatmap renders.


## [2.7.12] - 2026-08-31
### Fixed
- **Six promises were started and then dropped on the floor**, so anything that went wrong inside them vanished as an unhandled rejection. Each is now handled at the point where something can actually be done about it. The one with a user-visible consequence is the export preview: `readNoteBodies` had a `.then` that set the loaded bodies and cleared `isLoading`, and no rejection path — a single unreadable note in the selected range left the preview spinning forever with no explanation. It now logs the failure and clears the loading state either way. The rest are quieter: `i18n.changeLanguage` in `ReactApp` and `i18n.init` in `src/localization/i18n.ts` log a failure instead of discarding it, the add-colour handler in the palette settings now `await`s `saveSettings()` like the three handlers beside it already did, and the box click keeps its fire-and-forget shape — `handleBoxClick` catches and notifies internally, so that one is an explicit `void` rather than a new handler.
- **Three test mocks returned `undefined` from `changeLanguage`**, which the real i18next types as `Promise<TFunction>`. Nothing depended on the difference until the caller started handling the rejection, at which point seven `ReactApp` tests failed on `Cannot read properties of undefined (reading 'catch')`. The mocks now resolve a promise, so they model the API they stand in for.

### Changed
- **Settings headings and containers now go through the Obsidian APIs meant for them**, matching what the plugin guidelines expect and what the rest of the settings tab already looked like. The two section headings — "tabs visibility" and the support card's title — were raw `h3` and `h5` elements; they are now `new Setting(...).setName(...).setHeading()`, and the support one keeps its separator styling through `setClass()` rather than a `cls` on the element. Seven `createEl("div", ...)` calls in `src/settings.ts` and `src/settings/palette.settings.ts` became `createDiv(...)`. One visual consequence worth naming: the support card is `text-align: center`, but a `.setting-item` lays its name out with flex, so that title now reads left-aligned rather than centred. Its stylesheet rule gained `border-top: none`, since `.setting-item` draws a top border of its own that would otherwise double up against the existing `border-bottom`.
